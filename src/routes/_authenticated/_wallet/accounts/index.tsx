			...
			<Card className="rounded-2xl">
				...
				<CardContent>
					<form
						...
						className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1.5fr_auto]"
					>
						<Input
							value={name}
							onChange={(e) => setName(e.target.value)}
							placeholder="Account name (e.g. bKash, DBBL, Cash)"
				...
				<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{accounts.map((account) => (
						<AccountCard
							...
							onAdjust={() => {
								setAdjustingId(adjustingId === account.id ? null : account.id);
								setEditingId(null);
							}}
...
function AccountCard({
	...
	function handleCardClick(event: React.MouseEvent) {
		...
		navigate({
			to: "/apps/wallet/accounts/$accountId",
	...
	return (
		<Card
			...
			className={`feature-card rounded-2xl transition hover:-translate-y-0.5 ${editing || adjusting ? "" : "cursor-pointer"}`}
		>
			<CardHeader className="pb-0">
				<div className="flex min-w-0 flex-col gap-1.5">
					<CardTitle className="truncate text-base">{account.name}</CardTitle>
					<span
						className={`inline-flex w-fit items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${ACCOUNT_TYPE_COLORS[account.type]}`}
					>
				...
				<CardAction>
					<div ref={menuRef} className="relative">
						...
						{menuOpen && (
							<div
								role="menu"
								className="absolute right-0 top-full z-50 mt-2 w-48 overflow-hidden rounded-xl border border-[var(--line)] bg-[var(--menu-solid)] p-1.5 text-[var(--sea-ink)] shadow-[0_18px_44px_var(--shadow-ink-20),0_6px_20px_var(--shadow-soft-10)]"
							>
								<Link
									to="/apps/wallet/accounts/$accountId"
									params={{ accountId: account.id }}
